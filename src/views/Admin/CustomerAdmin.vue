          ...
          <template v-slot:item="{ item }">
            <tr>
              ...
              <td class="border-right border-bot" style="width: 250px;">
                <v-btn color="yellow" small class="mr-4" @click="UpdateCostomer(item)">Update</v-btn>
                <v-btn color="red" small @click="DeleteCostomer(item)">Delete</v-btn>
              </td>
            </tr>
          </template>
        </v-data-table>
...
import { VDataTable } from 'vuetify/lib/components/index.mjs';
  ...
  export default {
    ...
    computed: {
      ...mapGetters('costomerModules', ['getListData']),
    },
    created() {
      this.fetchListData();
    }
